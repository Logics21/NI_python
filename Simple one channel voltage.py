...
class voltageContinuousInput(tk.Frame):
    ...
    def runTask(self):
        ...
        if(self.samplesAvailable >= int(self.sampleRate/self.refreshRate)):
            ...
            self.valBuffer1.extend(self.tempData)
    ...
    def saveData(self):
        ...
        self.dataOutput = np.vstack((np.array(self.timeVec), np.array(self.valBuffer1))).T
        self.dataOutput = pd.DataFrame(self.dataOutput, columns = ["Time [ms]", "ch 0"])

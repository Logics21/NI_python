...
class VoltageContinuousInput(tk.Frame):
    ...
    def save_data(self):
        ...
        data_output = pd.DataFrame(data_output, columns=["Time [ms]", "ch 0"])
...
class ChannelSettings(tk.LabelFrame):
    ...
    def create_widgets(self):
        ...
        self.min_voltage_label.grid(row=4, column=0, sticky='w', padx=self.x_padding, pady=(10, 0))

        self.min_voltage_entry = ttk.Entry(self, width=10)
        self.min_voltage_entry.insert(0, "-10")
        self.min_voltage_entry.grid(row=5, column=0, sticky="ew", padx=(30,5))
        
        self.max_voltage_label = ttk.Label(self, text="Max Voltage")
        self.max_voltage_label.grid(row=4, column=1, sticky='w', padx=self.x_padding, pady=(10, 0))

        self.max_voltage_entry = ttk.Entry(self, width=10)
        self.max_voltage_entry.insert(0, "10")
        self.max_voltage_entry.grid(row=5, column=1, sticky="ew", padx=(5,30))

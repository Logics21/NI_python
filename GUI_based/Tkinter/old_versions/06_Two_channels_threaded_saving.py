...
class VoltageContinuousInput(tk.Frame):
        ...
        if samples_available >= int(self.sample_rate / self.refresh_rate):
            ...
            if self.recording_active:
                with self.buffer_lock:
                    ...
                    time_vec = np.arange(self.acquired_samples / (self.sample_rate / 1000), 
                                         (self.acquired_samples + n_samples) / (self.sample_rate / 1000), 
                                         1000 / self.sample_rate)
                    self.acquired_samples += n_samples
                    formatted_data = list(zip(time_vec, temp_data[0], temp_data[1]))                    
    ...
    def write_data_to_file(self):
        schema = pa.schema([("time_ms", pa.float64()), ("ch1", pa.float64()), ("ch2", pa.float64())])
        ...
        with pq.ParquetWriter(self.filepath, schema) as writer:
            while not self.stop_writing.is_set():
                ...
                df = pd.DataFrame(data_chunk, columns=["time_ms", "ch1", "ch2"])
